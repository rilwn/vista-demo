...
} from './saved-service-reports.dto.js';

interface SavedRow {
  id: string;
  name: string;
  configuration: Omit<SavedServiceReport, 'id' | 'name'>;
  request_hash: string;
}

@Injectable()
export class SavedServiceReportsService {
  constructor(
  ...
  async list(
  ...
  ): Promise<SavedServiceReportPageDto> {
    const rows = await this.database.getPool().query<SavedRow>(
      `SELECT id, name, configuration FROM reporting.saved_service_reports
    ...
    const count = await this.database
      .getPool()
      .query<{ total: number }>(
        'SELECT count(*)::integer AS total FROM reporting.saved_service_reports WHERE owner_account_id = $1',
        [auth.accountId],
      );
  ...
  async save(
    ...
    const available = await this.exports.serviceDefinitions();
    ...
    const configuration: SavedRow['configuration'] = {
      ...
      columns: validateReportColumns(
        input.definitionKey,
        input.columns ?? serviceReportColumns(input.definitionKey).map((column) => column.key),
      ),
    ...
    try {
      ...
      const inserted = await client.query(
        `INSERT INTO reporting.saved_service_reports (id, owner_account_id, name, configuration, request_hash)
         VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`,
        [input.id, auth.accountId, name, configuration, hash],
      );
      if (!inserted.rowCount) {
        const existing = await client.query<SavedRow>(
          'SELECT id, name, configuration, request_hash FROM reporting.saved_service_reports WHERE id = $1 AND owner_account_id = $2',
...
function mapSaved(row: SavedRow): SavedServiceReport {
