...
import { crmReportDefinitionKeys } from '@vista/contracts';
...
describe('CRM report field selection', () => {
  it.each(crmReportDefinitionKeys)('validates allowed fields for %s', (key) => {
    const columns = crmReportColumns(key);
  ...
  it.each(['csv', 'xlsx', 'pdf'] as const)('renders selected CRM fields in %s', async (format) => {
    const selected = selectReportColumns(
      {
        columns: crmReportColumns('crm.customer-value'),
        rows: [
          {
            period: 'Selected period',
            activeCustomers: 7,
            observedLifetimeValueBgn: '120.0000',
          },
        ],
      ...
      ['period', 'activeCustomers'],
    );
    expect(selected.columns.map((column) => column.label)).toEqual(['Window', 'Active customers']);
    const rendered = await renderFinanceReport(format, selected);
    if (format === 'csv') {
      expect(rendered.buffer.toString()).toContain('Selected period');
      ...
      expect(values).toContain('Selected period');
      expect(values).toContain(7);
      expect(values).not.toContain('Observed customer value BGN');
