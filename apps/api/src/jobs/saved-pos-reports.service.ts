...
import type { SavedPosReportDto, SavedPosReportPageDto } from './saved-pos-reports.dto.js';

interface SavedRow {
  id: string;
  name: string;
  configuration: Omit<SavedPosReport, 'id' | 'name'>;
  request_hash: string;
}

@Injectable()
export class SavedPosReportsService {
  constructor(
  ...
  async list(
  ...
  ): Promise<SavedPosReportPageDto> {
    const rows = await this.database.getPool().query<SavedRow>(
      `SELECT id, name, configuration FROM reporting.saved_pos_reports
    ...
    const count = await this.database
      .getPool()
      .query<{ total: number }>(
        'SELECT count(*)::integer AS total FROM reporting.saved_pos_reports WHERE owner_account_id = $1',
        [auth.accountId],
      );
  ...
  async save(
  ...
  ): Promise<SavedPosReport> {
    if (
      input.shiftId ||
      input.definitionKey === 'pos.x-report' ||
      input.definitionKey === 'pos.z-report'
    )
      throw new ApiErrorException(
        'REPORT_SAVE_NOT_AVAILABLE',
        'X and Z reports are opened from their cashier shift.',
        400,
      );
    const available = await this.exports.posDefinitions();
    ...
    const configuration: SavedRow['configuration'] = {
      ...
      columns: validateReportColumns(
        input.definitionKey,
        input.columns ?? posReportColumns(input.definitionKey).map((column) => column.key),
      ),
    ...
    try {
      ...
      const inserted = await client.query(
        `INSERT INTO reporting.saved_pos_reports (id, owner_account_id, name, configuration, request_hash)
         VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`,
        [input.id, auth.accountId, name, configuration, hash],
      );
      if (!inserted.rowCount) {
        const existing = await client.query<SavedRow>(
          'SELECT id, name, configuration, request_hash FROM reporting.saved_pos_reports WHERE id = $1 AND owner_account_id = $2',
...
function mapSaved(row: SavedRow): SavedPosReport {
